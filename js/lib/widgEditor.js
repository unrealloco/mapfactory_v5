...
var widgStylesheet = "css/widgContent.css?v1";
	...
	if (typeof(document.designMode) == "string" && (document.all || document.designMode == "off"))
	{
		...
		for (var i = 0; i < theTextareas.length; i++)
		{
			...
			if (theTextarea.className.classExists("widgEditor"))
			{
				...
				setTimeout("widg[" + widg.length + "] = new widgEditor('" + theTextarea.id + "')", 10 * (i));
			}
		}
	}
	else
...
function widgEditor(replacedTextareaID)
{
	...
	this.theTextarea.addClassName('WidgTextarea');
...
widgEditor.prototype.cleanPaste = function()
{
	if (
		widgAutoClean
		//~ || confirm("Do you wish to clean the HTML source of the content you just pasted?")
	){
		var matchedHead = "";
		var matchedTail = "";
		...
		newSnippet = newSnippet.stripScripts ().stripTags ();
		...
		//~ this.theInput.value = this.theInput.value.stripScripts ().stripTags ();
