<?php
	...
	class mysqlDatabase
	{
		...
		function query ($query){
			...
			$query = preg_replace ('#FROM[\t\r\n\v\f\s]+(\w+)#isU', 'FROM '.DB_PRE.'$1', $query);
			$query = preg_replace ('#UPDATE[\t\r\n\v\f\s]+(\w+)#isU', 'UPDATE '.DB_PRE.'$1', $query);
			$query = preg_replace ('#INTO[\t\r\n\v\f\s]+(\w+)#isU', 'INTO '.DB_PRE.'$1', $query);
			$query = preg_replace ('#JOIN[\t\r\n\v\f\s]+(\w+)#isU', 'JOIN '.DB_PRE.'$1', $query);
